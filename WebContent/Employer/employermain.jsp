...
<div class="container">
		...
		<div class="navbar-collapse collapse"
			...
			<ul class="nav navbar-nav">
				...
				<li><a href="searchcandidate.jsp">Search Candidates</a></li>
	...
	<div class="container">
		<div class="single">
			...
			<a href=<%=companydetails.getCompanyLink()%>>
				...
				<div class="row" style="margin: 5%; width: 90%; height: 20%; border: solid 1px grey; padding: 10px;" >
					<div class="form-group col-md-12">
						<label class="col-md-9 control-lable" for="Jobprofile"><h3><%=jobdetails.getJobProfile()%></h3></label>
						<div class="form-actions floatRight">
							<label class="col-md-0 control-lable"
								for="Job-Post-Date-And-Time"><%=rs.getDate("activitydate")%>
								: <%=rs.getTime("activitytime")%></label>
						</div>
					</div>
			...
			<div class="row"
				...
				<div class="form-group col-md-12">
					<form class="col-md-3 control-lable" method="get" action="../SelectCandidate">
								<input type="hidden" name="userid" value=<%= userdetails.getUserid() %>>
								<input type="hidden" name="jobid" value=<%= rs.getInt("activityjobid")%>>
						...
						<form class="col-md-3 control-lable" method="get" action="../ReviewCandidate">
								<input type="hidden" name="userid" value=<%= userdetails.getUserid()%>>
								<input type="hidden" name="jobid" value=<%= rs.getInt("activityjobid")%>>
								<input type="hidden" name="empid" value=<%= companydetails.getCompanyID() %>>
								<input type="submit" class="btn btn-primary btn-sm" value="Review">
							</form>
						<form class="col-md-3 control-lable" method="get" action="../RejectCandidate">
								<input type="hidden" name="userid" value=<%= userdetails.getUserid() %>>
								<input type="hidden" name="jobid" value=<%= rs.getInt("activityjobid")%>>
								<input type="hidden" name="empid" value=<%= companydetails.getCompanyID() %>>
								<input type="submit" class="btn btn-primary btn-sm" value="Reject">
						</form>
				</div>
			</div>
